...
<form  action="@if(isset($product)) {{route('admin.product.update', $product)}} @else {{route('admin.product.store')}} @endif" method="post" accept-charset="utf-8" enctype="multipart/form-data">
		...
		<div class="col-lg-9">
			<div class="form-group row">
				...
				<div class="col-lg-12">
					...
					<input type="text" id="txturl" name="title" class="form-control " value="{{old('title')}}{{@$product->title}}" />
					<p class="small">{{config('app.url')}}/<span id="url">{{old('title')}}{{@$product->slug}}</span>
		...
		<div class="form-group row">
			...
			<div class="col-6">
				...
				<div class="input-group mb-3">
					...
					<input type="text" class="form-control" name="discount_price" placeholder="0.00" aria-label="discount_price" aria-describedby="discount" value="{{old('discount_price')}}{{@$product->discount_price}}" />
				</div>
			</div>
		</div>
		<div class="form-group row">
	...
	<div class="col-lg-3">
		<ul class="list-group row">
			...
			@php
			$ids = (isset($product) && $product->categories->count() > 0 ) ? $product->categories->pluck('id')->toArray() : null;
			@endphp
