		...
		<div class="col-lg-9">
			<div class="form-group row">
				...
				<div class="col-sm-12 col-md-6">
					...
					<input type="text" id="txturl" name="name" class="form-control " value="{{old('name')}}{{@$user->profile->name}}" />
					<p class="small">{{route('admin.profile.index')}}/<span id="url">{{old('name')}}{{@$user->profile->slug}}</span>
			...
			<div class="col-sm-6">
				...
				<div class="input-group mb-3">
					<select class="form-control" id="status" name="status">
						<option value="0" {{ old('status') == 0 ? 'selected' : '' }} @if(isset($user) && $user->status == 0) {{'selected'}} @endif >Blocked</option>
						<option value="1" {{ old('status') == 1 ? 'selected' : '' }} @if(isset($user) && $user->status == 1) {{'selected'}} @endif>Active</option>
					</select>
				</div>
			</div>
			@php
